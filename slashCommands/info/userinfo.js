...
module.exports = {
    ...
    run: async(bot, interaction, args) => {
        ...
        const target = args.getUser("target") || interaction.user;
        const member = await guild.members.cache.get(target.id) || await guild.members.fetch(target.id)
        const flags = {
...
embed.setFooter(`Requested by ${interaction.user.tag}`, interaction.user.displayAvatarURL({dynamic: true, format: "png", size: 4096}))
