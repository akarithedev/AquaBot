...
module.exports = {
  ...
  run: async (bot, interaction, args) => {
    ...
    const member =
      ...
      (await guil.members.fetch(interaction.user.id));
    ...
    try {
      ...
      if (player.voiceChannel !== voiceChannel.id) {
        embed.setDescription(
          `${bot.emoji.error} I'm being used in another voice channel.`
        );
        embed.setColor("BLUE");
        embed.setFooter(
          `Requested by ${interaction.user.tag}`,
      ...
      if (res.loadType === "NO_MATCHES") {
        ...
        embed.setDescription(
          `${bot.emoji.error} I couldn't found any song called: \`${songs}\``
        );
        embed.setColor("BLUE");
        embed.setFooter(
          `Requested by ${interaction.user.tag}`,
      ...
      if (res.loadType === "PLAYLIST_LOADED") {
        ...
        if (
          ...
          player.play();
        embed.setDescription(
          `${bot.emoji.success} Added playlist \`${res.playlist.name}\` to queue.`
        );
        embed.setColor("BLUE");
        embed.setFooter(
          `Requested by ${interaction.user.tag}`,
      ...
      if (player.queue.length !== 0) {
        ...
        setTimeout(() => {
          ...
          embed.setDescription(
            `${bot.emoji.success} Successfully added **[${res.tracks[0].title}](${res.tracks[0].uri})** to the queue.`
          );
          embed.setColor("BLUE");
          embed.setFooter(
            `Requested by ${interaction.user.tag}`,
